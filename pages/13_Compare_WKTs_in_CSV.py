...
st.markdown("""
    **Instructions:** Upload a CSV file and select the two columns that contain the Well-Known Text (WKT) for the polygons you want to compare.
    You can also select the starting row and the number of rows to process (up to a maximum of 100).
...
if uploaded_file is not None:
    ...
    num_rows = st.slider('Select the number of rows to process', min_value=1, max_value=min(100, len(df)-start_row), value=5, step=1)
